...
class ESA_layer(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        self.conv1 = nn.Conv2d(dim, inner_dim, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(dim, inner_dim, kernel_size=5, padding=2)
        self.conv3 = nn.Conv2d(dim, inner_dim, kernel_size=7, padding=3)
    ...
    def forward(self, x, mask):
        ...
        k2, v2 = self.ppm(k2), self.ppm(v2)  # k/v shape: (b, inner_dim, n_kv)

        k2 = rearrange(k2, 'b (head d) n -> b head n d', head=self.heads) # k shape: (b, head, n_kv, d)
        v2 = rearrange(v2, 'b (head d) n -> b head n d', head=self.heads) # v shape: (b, head, n_kv, d)
        k1, v1 = MaskAveragePooling(k1, mask), MaskAveragePooling(v1, mask)
        k1 = rearrange(k1, 'b (head d) n -> b head n d', head=self.heads)  # k shape: (b, head, n_kv, d)
        v1 = rearrange(v1, 'b (head d) n -> b head n d', head=self.heads)  # v shape: (b, head, n_kv, d)
        k = k1 + k2
        v = v1 + v2
